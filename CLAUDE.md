...
- **Hamel workflow: run → look → categorize → grade. No graders before trace review.** Run the agent against the dataset first, read every trace yourself, judge pass/fail by gut, write free-text notes about what went wrong, group those notes into failure categories, *then* build graders for the categories that matter. This applies to every grader, including ones that look like simple field comparisons against `expected` — defer them until trace review has happened. The `expected` field in the dataset is the ground-truth answer key for that step; it's not a license to encode graders before you've looked at traces.
- **Code-based grader first, LLM judge second.** Orthogonal to the workflow order above: once you know *what* to grade, prefer code (schema validator, regex, tool-arg comparison) over a judge. Reserve judges for genuinely subjective calls (urgency, tone, faithfulness), and validate them with TPR/TNR > 90% on a held-out set before trusting them. See `plan/my_evals_work.md` for the reasoning.

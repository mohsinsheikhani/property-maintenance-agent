"""Component eval for the classify node.

Calls `agent.graph.nodes.classify` live, per dataset record, and applies code
graders to the result. No graph, no run.jsonl, no MCP server needed; classify
only reads from_address/subject/body from state.

Skips records where expected.classify.urgency is absent (archive and
not-a-maintenance-request cases).

Usage:
    uv run python -m evals.components.classify_eval
    uv run python -m evals.components.classify_eval --dataset datasets/e2e/dev.jsonl --limit 5
...
async def run(dataset_path: Path, limit: int | None, only_id: str | None) -> None:
    ...
    print(f"Running classify on {len(records)} records from {dataset_path}\n")
